        ...
        <div className="flex flex-col items-start w-full">
            ...
            <div className={`max-w-[80%] p-2`} style={{ backgroundColor: '#e5e7eb', color: textColor, borderRadius: `${messageBubbleRadius}px`, ...fontStyles }}>
              ...
              {hasSources && (
                <div className="mt-2 pt-2 border-t border-gray-300">
                  <div className="text-xs font-semibold mb-1 opacity-70">📚 Sources ({message.sources!.length}):</div>
          ...
          {!hasFeedback && onSubmitMessageFeedback && (
            <div className="mt-1 flex gap-2" style={{ marginLeft: (showMessageAvatars && widgetConfig?.bot_avatar) ? '40px' : '0' }}>
              <button onClick={() => onSubmitMessageFeedback(message.id, 'thumbs_up')} className="text-xs opacity-50 hover:opacity-100 transition-opacity flex items-center gap-1" style={{ color: textColor }} title="Thumbs up">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" /></svg>
              </button>
              <button onClick={() => onSubmitMessageFeedback(message.id, 'thumbs_down')} className="text-xs opacity-50 hover:opacity-100 transition-opacity flex items-center gap-1" style={{ color: textColor }} title="Thumbs down">
          ...
          {hasFeedback && (
            <span className="mt-1 text-xs opacity-50" style={{ color: textColor, marginLeft: (showMessageAvatars && widgetConfig?.bot_avatar) ? '40px' : '0' }}>Feedback submitted</span>
          )}
          {showTimestamps && message.timestamp && (
            <span className="mt-1 text-xs opacity-50" style={{ color: textColor, marginLeft: (showMessageAvatars && widgetConfig?.bot_avatar) ? '40px' : '0' }}>{new Date(message.timestamp).toLocaleTimeString()}</span>
          )}
        </div>
      </div>
    );
